...
class GemmaVisionService
  ...
  class << self
    def analyze_gender(photo)
      ...
      # Send to Ollama/Gemma for analysis
      response = query_gemma(face_image_base64)

      # Parse the response
      parse_gender_response(response, photo.id)
    rescue => e
      Rails.logger.error "GemmaVisionService error for photo #{photo.id}: #{e.message}"
      nil
    end
    ...
    def get_face_crop_base64(photo)
      ...
      p "Downloading face crop for photo #{photo.id}"
    ...
    def parse_gender_response(response_text, photo_id)
      ...
      begin
        ...
        Rails.logger.error "Failed to parse Gemma response for photo #{photo_id}: #{e.message}"
